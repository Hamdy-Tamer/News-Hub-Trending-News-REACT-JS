...
export default function Business() {
  ...
  async function fetchBusinessNews() {
    try {
      ...
      let { data } = await axios.get(`https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=061c3bff2b054e75b3cca1dae6df9835`);
      console.log(data);
  ...
  if (error) {
    return (
      <div className={styles.errorContainer}>
        ...
        <button className={styles.retryButton} onClick={fetchBusinessNews}>
          Try Again
        </button>
      </div>
    );
        ...
        <div className="container">
          <div className="row">
            <div className="col-12 text-center">
              <h1 className={styles.heroTitle}>Business News</h1>
              <p className={styles.heroSubtitle}>
                Your daily briefing on the world of commerce. Track the stock market, follow major corporate deals, 
                and get insights into the economy, startups, and leadership trends that move markets.
              </p>
            </div>
          </div>
        </div>

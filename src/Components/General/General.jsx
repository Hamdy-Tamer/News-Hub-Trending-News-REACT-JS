...
export default function General() {
  ...
  async function fetchGeneralNews() {
    try {
      ...
      let { data } = await axios.get(`https://newsapi.org/v2/top-headlines?country=us&category=general&pageSize=12&apiKey=061c3bff2b054e75b3cca1dae6df9835`);
      console.log(data);
      console.log(data.articles);
      setNews(data.articles);
    } catch (err) {
      console.error("Failed to fetch news:", err);
      setError("Failed to load general news. Please try again later.");
    } finally {
      setLoading(false);
    }
  }
  ...
  if (error) {
    return (
      <div className={styles.errorContainer}>
        ...
        <button className={styles.retryButton} onClick={fetchGeneralNews}>
          Try Again
        </button>
      </div>
    );
        ...
        <div className="container">
          <div className="row">
            <div className="col-12 text-center">
              <h1 className={styles.heroTitle}>General News</h1>
              <p className={styles.heroSubtitle}>
                Stay informed with the latest headlines and breaking news from around the world. 
                Comprehensive coverage of the most important stories across all topics.
              </p>
            </div>
          </div>
        </div>
